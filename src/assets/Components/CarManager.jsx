        ...
        return (
          <div key={car._id} className="car-box">
            <div className="car-box-header">
              <div
                className="car-box-title"
                onClick={() => toggleCarExpand(car._id)}
                style={{ cursor: 'pointer', fontWeight: 600, fontSize: '1.1rem' }}
              >
                <span style={{ marginRight: 12 }}>{isExpanded ? '▼' : '▶'}</span>
                <b>STT: </b> <span style={{ fontSize: '1.3rem', marginRight: 12 }}>{index + 1}</span> | <b>Biển số:</b> {car.plateNumber} | {car.carType?.name}
              </div>
